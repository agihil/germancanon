...
## Additional Remarks Section IV.1
When calculating the correlation between the primary indicators, Spearman’s rank correlation between the resulting correlation values and the difference in indicator length is -0.62, indicating that larger differences in length between two indicators correspond to lower correlations. When the correlation is calculated only on the intersection, Spearman’s rank correlation between the resulting correlation values and the difference in indicator length drops to -0.24. This shows that the intersection-based correlation is less strongly influenced by differences in length between primary indicators than the raw correlation.
